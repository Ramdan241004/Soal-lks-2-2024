...
Anda harus membuat dan mengatur custom VPC dengan nama speaks-vpc dan CIDR 192.168.10.0/24.
Pastikan DNS hostnames diaktifkan dan VPC memiliki tiga availability zones dengan rincian:
...
4. Dua private subnet masing-masing 64 host:

speaks-privsubnet-a di us-east-1a

speaks-privsubnet-b di us-east-1b
...
1. Luncurkan 3 instance EC2:

1 control-plane (lks-master-node)

2 worker (lks-node1, lks-node2)
...
6. Deploy cluster Docker Swarm di lks-master-node dan node1, node2.


7. Daftarkan semua instance sebagai GitHub self-hosted runner:

Label master untuk control plane,

Label node1 untuk lks-node1,

Label node2 untuk lks-node2.
...
Salin JSON task definition dan simpan di masing-masing folder layanan dengan nama task-definition.json.
...
2. Buat subscription dengan ARN topic di atas,

Protocol: Email

Endpoint: personallabs4@gmail.com

Konfirmasi ke juri jika status subscription masih pending.



3. Publikasikan pesan berikut ke topic:

Subject: Hello from <Provinsi>!

Message body: New version The Voting App from <Provinsi> is Released!!
